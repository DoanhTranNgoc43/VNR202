...
export default function ArtPage() {
  ...
  const totalPages = Math.ceil(filteredArts.length / ITEMS_PER_PAGE)
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
  const endIndex = startIndex + ITEMS_PER_PAGE
  const paginatedArts = filteredArts.slice(startIndex, endIndex)
  ...
  const uniqueCategories = useMemo(() => {
    const categories = Array.from(new Set(ART_ITEMS.map(item => item.category)))
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#120c06] via-[#1a1009] to-[#2a1a0c] text-white">
      ...
      <section className="relative">
        <div className="absolute inset-0">
          <img 
            src="https://bvhttdl.mediacdn.vn/zoom/600_315/2021/1/28/dan-trung-1611826501304704364989-crop-16118265123241222963276.png" 
            alt="Nghệ thuật Việt Nam" 
          ...
          <div className="flex-1 flex items-center">
            <div className="text-white space-y-4">
              <div className="flex items-center gap-3">
                <Music className="w-10 h-10 md:w-12 md:h-12 text-amber-400" />
                <h1 className="text-4xl md:text-6xl font-serif font-bold">Nghệ Thuật Việt Nam</h1>
              </div>
              <h2 className="text-xl md:text-2xl text-white/90">Nhạc cụ, sân khấu, lễ hội</h2>
              <p className="max-w-2xl text-white/85">
                Khám phá nền nghệ thuật đa dạng và phong phú của Việt Nam, từ các nhạc cụ dân tộc độc đáo đến các loại hình sân khấu truyền thống và các lễ hội văn hóa đặc sắc.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 py-8 sticky top-0 z-40 bg-gradient-to-b from-[#1a1009] to-transparent backdrop-blur-sm">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="relative flex-1 w-full md:max-w-md">
            ...
            <Input
              type="text"
              placeholder="Tìm kiếm nghệ thuật, mô tả hoặc nguồn gốc..."
              value={searchQuery}
        ...
        <div className="mt-4 text-white/70 text-sm">
          Hiển thị {filteredArts.length} / {ART_ITEMS.length} nghệ thuật
          {filteredArts.length > ITEMS_PER_PAGE && (
            <span className="ml-2">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 py-8">
        {filteredArts.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-white/70 text-lg">Không tìm thấy nghệ thuật nào</p>
            <p className="text-white/50 text-sm mt-2">Thử tìm kiếm với từ khoá khác hoặc chọn loại nghệ thuật khác</p>
          </div>
        ) : (
          <>
                  ...
                  <div className="relative h-48 overflow-hidden">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        target.src = "https://via.placeholder.com/400x300/1a1009/ffffff?text=" + encodeURIComponent(art.name)
                      }}
                    />
